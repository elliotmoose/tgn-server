...
const johnCredentials = {
	username: "john",
	fullName: "John Doe",
	bio: 'I like to stalk people',
	email: "john@gmail.com",
	password: "12345"
}

const johnsonCredentials = {
	username: "johnson",
	fullName: "Johnson Lee",
...
export const updateOrganisation = async function (organisationKey, orgUpdateData, user) {
    let organisation = organisations[organisationKey];
    let updateOrgRes = await chai.request(server).put(`/organisations/${organisation._id}`).set('authorization', `Bearer ${user.token}`).send(orgUpdateData);
    updateOrgRes.should.have.status(200);
    organisations[organisationKey] = {...organisation, ...updateOrgRes.body.data};
}
...
export const canViewPostAs = async function (user, post) {
    let res = await chai.request(server).get(`/posts/${post._id}`).set('authorization', `Bearer ${user.token}`).send();
